...
resource "aws_s3_bucket" "chewbacca_ir_reports_bucket01" {
  bucket = "${local.Environment}-ir-reports-${data.aws_caller_identity.current.account_id}"

  tags = {
    Name = "${local.Environment}-ir-reports-bucket01"
  }
}
...
resource "aws_iam_role" "chewbacca_ir_lambda_role01" {
  name = "${local.Environment}-ir-lambda-role01"
...
resource "aws_iam_policy" "chewbacca_ir_lambda_policy01" {
  name = "${local.Environment}-ir-lambda-policy01"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Parameter Store
      {
        ...
        Resource = "arn:aws:ssm:*:${data.aws_caller_identity.current.account_id}:parameter${var.parameter_location}*"
      },
      # Secrets Manager
      {
        Effect = "Allow",
        ...
        Resource = "arn:aws:secretsmanager:*:${data.aws_caller_identity.current.account_id}:secret:${var.secret_location}*"
      },
      # S3 report write
      {
        Effect = "Allow",
...
# resource "aws_sns_topic" "star_topic_ai" {
#   name = "bedrock_report"
# }
# resource "aws_sns_topic_subscription" "email_subscription2" {
#   topic_arn = aws_sns_topic.star_topic_ai.arn
#   protocol  = "email"
...
resource "aws_lambda_function" "chewbacca_ir_lambda01" {
  function_name = "${local.Environment}-ir-reporter01"
  ...
  environment {
    variables = {
      ...
      APP_LOG_GROUP    = "/lab/${local.Environment}/ec2/health"
      WAF_LOG_GROUP    = "aws-waf-logs-${local.Environment}-webacl01"
      SECRET_ID        = "${var.secret_location}"
      SSM_PARAM_PATH   = "${var.parameter_location}"
      BEDROCK_MODEL_ID = "amazon.nova-2-lite-v1:0" # TODO: students choose a Bedrock text model id available in their account/region
...
resource "aws_lambda_permission" "chewbacca_allow_sns_invoke01" {
  ...
  source_arn    = aws_sns_topic.health_check_topic.arn
}
